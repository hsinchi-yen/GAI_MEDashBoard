...
def fetch_index_yoy(ticker: str, years_back: int = 12) -> pd.DataFrame:
    """
    通用股市指數月頻 YoY (%) — Yahoo Finance v8 API。
    ticker: Yahoo Finance 代號，如 '^N225'、'^KS11'、'^HSI'、'000300.SS'。
    """
    try:
        ...
        print(f"Index YoY fetch error ({ticker}): {e}")
        return pd.DataFrame()


# ── 13F Smart Money (SEC EDGAR) ─────────────────────────────────────────────
...
_13F_MAJOR_FUNDS: dict[str, str] = {
    # CIKs verified against SEC EDGAR submissions API (2026-05-10)
    "Berkshire Hathaway":  "0001067983",  # Berkshire Hathaway Inc
    "Citadel Advisors":    "0001423053",  # Citadel Advisors LLC
    "Two Sigma":           "0001179392",  # Two Sigma Investments LP
    "Millennium Mgmt":     "0001273087",  # Millennium Management LLC
    "D.E. Shaw":           "0001009207",  # D.E. Shaw & Co.
    "AQR Capital":         "0001167557",  # AQR Capital Management LLC
    "Point72":             "0001603466",  # Point72 Asset Management LP
    "Tiger Global":        "0001167483",  # Tiger Global Management LLC
    "Viking Global":       "0001101785",  # Viking Global Equities LP
    "Lone Pine Capital":   "0001061165",  # Lone Pine Capital LLC
    "Pershing Square":     "0001336528",  # Pershing Square Capital Management
    "Third Point":         "0001040273",  # Third Point LLC
    "Baupost Group":       "0001054420",  # Baupost Group LLC
    "Appaloosa Mgmt":      "0001006438",  # Appaloosa Management LP
    "Coatue Management":   "0001135730",  # Coatue Management LLC
    "Greenlight Capital":  "0001079114",  # Greenlight Capital Inc
    "Maverick Capital":    "0000934639",  # Maverick Capital Ltd
    "Eminence Capital":    "0001107310",  # Eminence Capital LP
    "Glenview Capital":    "0001138995",  # Glenview Capital Management LLC
...
def _clean_issuer_name(name: str, maxlen: int = 30) -> str:
    """Normalize 13F nameOfIssuer: collapse whitespace, title-case, truncate."""
...
def _edgar_get_13f_accessions(cik: str, n: int = 2) -> list[dict]:
    """
    Return n most-recent distinct-period 13F accession numbers.
    Prefers 13F-HR/A (amendments) over 13F-HR for the same reportDate,
    so the latest filed version of each quarter is always used.
    """
    try:
        ...
        # Group by reportDate; keep latest filingDate per period (handles /A amendments)
        ...
        periods = sorted(by_period.values(), key=lambda x: x["reportDate"], reverse=True)
        return periods[:n]
    except Exception as e:
        print(f"EDGAR accessions error (CIK={cik}): {e}")
        return []


def _edgar_get_infotable_url(cik: str, accession: str) -> str | None:
    """
    Locate the infotable XML URL by fetching the filing's index.json.
    Handles multiple naming conventions observed in the wild:
      - 'infotable.xml', 'form13fInfoTable.xml'  (D.E. Shaw, Tiger Global, …)
      - 'informationtable.xml'                    (Two Sigma, …)
      - numeric names like '50240.xml'            (Berkshire, …)
    """
    try:
        cik_path   = str(int(cik))          # strip leading zeros for archive URLs
        ...
        # Priority 1 — filename contains a known infotable keyword
        for it in xml_items:
            n = it["name"].lower().replace("_", "").replace("-", "")
            if "infotable" in n or "informationtable" in n:
                return f"{base}/{it['name']}"

        # Priority 2 — any XML that is NOT the cover page / index
        ...
        print(f"EDGAR index error (CIK={cik}, acc={accession}): {e}")
        return None


def _edgar_parse_infotable(xml_url: str) -> list[dict]:
    """
    Download and parse a 13F infotable XML.
    Returns list of {cusip, name, shares, value_k} per holding (value in $K).
    Only SOLE and DFND investment discretion are counted to avoid double-counting.
    """
    try:
        res = requests.get(xml_url, headers=_EDGAR_HEADERS, timeout=30)
        res.raise_for_status()
        root = ET.fromstring(res.text)
        # Extract namespace from root tag (e.g. {http://www.sec.gov/...}) so
        # we can iterate correctly whether or not the file uses a default xmlns.
...
def _holdings_to_map(lst: list[dict]) -> dict[str, dict]:
    """
    Aggregate holdings list → {key: {name, shares, value_k}}.
    Primary key = CUSIP when available, else nameOfIssuer.
    Multiple rows for the same security (e.g. put + call wrappers) are summed.
    """
    m: dict[str, dict] = {}
    for h in lst:
...
def _fetch_fund_delta(fund_name: str, cik: str) -> tuple[dict, str]:
    """
    Fetch current and previous quarter 13F for one fund.
    Returns ({key: {name, share_delta, value_delta_k}}, report_date_str).
    Key is CUSIP when available, else nameOfIssuer.
    """
    try:
        ...
        print(f"Fund delta error ({fund_name}): {e}")
        return {}, ""


def fetch_13f_smart_money(n_top: int = 20) -> dict:
    """
    Aggregate net position changes from SEC EDGAR 13F-HR filings across 20 major hedge funds.
    Phase 2 improvements:
      - 13F-HR/A amendments handled (prefers latest filing per reportDate period)
      - CUSIP-based deduplication (avoids nameOfIssuer spelling variants)
      - Three ranking dimensions: share count, market value ($M), institutional conviction
      - Conviction = stocks where ≥2 institutions moved in the same direction
    First fetch ~20-30s; dashboard wraps in @st.cache_data(ttl=86400).
    """
    aggregate: dict[str, dict] = {}
    max_report_date = ""
    fund_count = 0

    def _fetch_one(item: tuple[str, str]) -> tuple[str, dict, str]:
        name, cik = item
        delta, rd = _fetch_fund_delta(name, cik)
        return name, delta, rd

    with ThreadPoolExecutor(max_workers=3) as pool:
        futures = {pool.submit(_fetch_one, item): item[0] for item in _13F_MAJOR_FUNDS.items()}
        for future in as_completed(futures):
            try:
                ...
                print(f"Smart money aggregation error: {e}")
    ...
    conviction_buy  = sorted(
        [(k, v) for k, v in buy_items  if v["funds_buy"]  >= 2],
        key=lambda x: (-x[1]["funds_buy"],  -x[1]["share_delta"])
    )[:n_top]
    conviction_sell = sorted(
        [(k, v) for k, v in sell_items if v["funds_sell"] >= 2],
        key=lambda x: (-x[1]["funds_sell"],  x[1]["share_delta"])
    )[:n_top]
    ...
    return {
        ...
        # (display_name, share_delta, n_funds)
        ...
        # (display_name, value_delta_usd_m, n_funds)
        ...
        # (display_name, n_funds, share_delta)
...
def fetch_vkospi(years_back: int = 6) -> pd.DataFrame:
    """
    韓國波動率指數 VKOSPI — Yahoo Finance ^VKOSPI（月頻收盤原始水位）。
    與 VIX 並排可觀察亞太整體恐慌程度及區域分化。
    """
    try:
        url = (
            "https://query1.finance.yahoo.com/v8/finance/chart/%5EVKOSPI"
            "?interval=1mo&range=7y"
        )
        headers = {
            "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64)",
            "Accept": "application/json",
        ...
        print(f"VKOSPI fetch error: {e}")
        return pd.DataFrame()


def edgar_connectivity_test() -> tuple[bool, str]:
    """
    Quick EDGAR reachability check using Berkshire's well-known submissions URL.
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            return True, f"SEC EDGAR 連線正常（已驗證：{name}）"
